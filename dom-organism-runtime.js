const DOMOrganismRuntime=(()=>{
  'use strict';
  const state={records:new Map(),sensors:new Map(),hazards:new Map(),hazardHistory:new Map(),physicsCache:new Map(),lastBatchAt:null,lastMode:null,lastSummary:null,lastHazardAt:null,lastEmitAt:0,emitTimer:null,sensorDirty:true,scienceTimer:null,scienceCursor:0};
  ...
  function pruneSensors(now=Date.now()){let removed=0;for(const[k,s]of state.sensors)if(sensorExpired(s,now)){state.sensors.delete(k);removed++}if(removed)state.sensorDirty=true;return removed}
  ...
  function acceptHazards(rows=[],mode='hazard'){
    let changed=0;for(const raw of rows||[]){if(!raw)continue;const h=canonicalHazard(raw),old=state.hazards.get(h.canonicalKey);const nt=timeValue(h.canonicalTime),ot=timeValue(old?.canonicalTime);if(!old||nt>=ot){state.hazards.set(h.canonicalKey,h);recordHazardHistory(h);state.physicsCache.delete(h.canonicalKey);changed++}}
    if(state.hazards.size>5000){const ordered=[...state.hazards.entries()].sort((a,b)=>timeValue(a[1].canonicalTime)-timeValue(b[1].canonicalTime));for(const[k]of ordered.slice(0,state.hazards.size-4500)){state.hazards.delete(k);state.physicsCache.delete(k)}}
    state.lastHazardAt=new Date().toISOString();state.lastMode=mode;scheduleScience();scheduleEmit();return changed
  }
  ...
  function canonicalSnapshot(){const hazards=[...state.hazards.values()].map(h=>({...h,physics:state.physicsCache.get(h.canonicalKey)||null,historyCount:(state.hazardHistory.get(h.canonicalKey)||[]).length}));const byKind={};for(const h of hazards){const k=String(h.kind||'Unknown');byKind[k]=(byKind[k]||0)+1}return{schema:'dom-canonical-planetary-organism-v1',updatedAt:new Date().toISOString(),recordCount:state.records.size,sensorCount:state.sensors.size,hazardCount:hazards.length,hazardKinds:byKind,lastObservationAt:state.lastBatchAt,lastHazardAt:state.lastHazardAt,hazards,historySeries:state.hazardHistory.size,physicsEvaluated:state.physicsCache.size,truthPolicy:'Observed, modeled, inferred and official states remain distinct. Exact source coordinates are preserved. Missing inputs stay missing. Future pathways are emitted only through equation-gated physics/model engines.'}}
  function emitNow(){state.emitTimer=null;state.lastEmitAt=Date.now();if(state.sensorDirty){const legacy=legacySnapshot();try{window.dispatchEvent(new CustomEvent('dom:organism-state',{detail:legacy}))}catch(_){ }state.sensorDirty=false}const canonical=canonicalSnapshot();try{window.dispatchEvent(new CustomEvent('dom:canonical-organism-state',{detail:canonical}))}catch(_){ }window.DOMCanonicalOrganismState=canonical;return canonical}
  function scheduleEmit(delay=180){if(state.emitTimer)return;const elapsed=Date.now()-state.lastEmitAt;const wait=Math.max(delay,700-elapsed);state.emitTimer=setTimeout(emitNow,wait)}
  function scienceStep(){state.scienceTimer=null;const keys=[...state.hazards.keys()];if(!keys.length)return;let processed=0;while(processed<6&&keys.length){if(state.scienceCursor>=keys.length)state.scienceCursor=0;const k=keys[state.scienceCursor++],h=state.hazards.get(k);if(h){state.physicsCache.set(k,physicsFor(h));processed++}if(state.scienceCursor===0)break}scheduleEmit(250);if(state.physicsCache.size<state.hazards.size)state.scienceTimer=setTimeout(scienceStep,90)}
  function scheduleScience(){if(state.scienceTimer)return;state.scienceTimer=setTimeout(scienceStep,350)}
  function accept(records=[],mode='unknown'){
    if(!ingressReady()){scheduleEmit();return canonicalSnapshot()}
    const clean=DOMObservationIngress.dedupe(records);let sensorChanged=false;
    ...
    if(state.sensors.size>15000){const ordered=[...state.sensors.entries()].sort((a,b)=>timeValue(a[1].receivedAt)-timeValue(b[1].receivedAt));for(const[k]of ordered.slice(0,state.sensors.size-13000))state.sensors.delete(k);sensorChanged=true}
    if(sensorChanged)state.sensorDirty=true;state.lastBatchAt=new Date().toISOString();state.lastMode=mode;state.lastSummary=DOMObservationIngress.summary(clean);scheduleEmit();return canonicalSnapshot()
  }
  ...
  function clear(){state.records.clear();state.sensors.clear();state.hazards.clear();state.hazardHistory.clear();state.physicsCache.clear();state.lastBatchAt=null;state.lastMode=null;state.lastSummary=null;state.lastHazardAt=null;state.sensorDirty=true;scheduleEmit(0);return organism()}
  function boot(){state.sensorDirty=true;scheduleEmit(0);if(typeof DOMObservationBroker!=='undefined')DOMObservationBroker.configureFromPage()}
  ...
  for(const n of ['dom:planetary-evidence-summary','dom:environmental-coupling-summary','dom:operational-weather-summary'])window.addEventListener(n,()=>{scheduleScience();scheduleEmit(300)});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else setTimeout(boot,0);
  return{accept,acceptHazards,snapshot,organism,history,renderPlan,clear,boot,newer,sensorExpired,pruneSensors,scheduleScience};
})();