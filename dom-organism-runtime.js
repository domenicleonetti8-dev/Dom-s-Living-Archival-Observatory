const DOMOrganismRuntime=(()=>{
  ...
  function accept(records=[],mode='unknown'){
    if(!window.DOMObservationIngress)return emit();
    const clean=DOMObservationIngress.dedupe(records);
    for(const r of clean){state.records.set(key(r),r);const packet=DOMObservationIngress.toSensor(r);if(packet&&window.DOMGlobalSensorGlobe){const sensor=DOMGlobalSensorGlobe.normalizeSensor(packet);if(sensor.id)state.sensors.set(`${sensor.lineageId}|${sensor.id}`,sensor)}}
  ...
  function renderPlan(options={}){if(!window.DOMGlobalSensorGlobe)return null;return DOMGlobalSensorGlobe.renderPlan({...options,sensors:[...state.sensors.values()]})}
