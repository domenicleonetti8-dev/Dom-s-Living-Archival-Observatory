...
function scheduleEnhancements(){if(scheduled||!window.DOMCurrentHazardMap?.map)return;scheduled=true;const m=window.DOMCurrentHazardMap.map;restoreMap(m);
  const critical=[
    ['./dom-hazard-popup-provenance.js?v=20260914-3','domHazardPopupProvenance'],
    ['./dom-earthquake-visual-restoration.js?v=20260914-4','domEarthquakeVisualRestoration'],
    ['./dom-weather-climate-station-inspector.js?v=20260914-3','domWeatherClimateStationInspector'],
    ['./dom-geographic-hazard-motion.js?v=20260914-7','domGeographicHazardMotion']
  ];
  critical.forEach((x,i)=>loadScript(x[0],x[1],80+i*120));
  const science=[
    ['./dom-planetary-evidence-fabric.js?v=20260914-3','domPlanetaryEvidenceFabric'],
    ['./dom-planetary-coupled-hazard-math.js?v=20260914-4','domPlanetaryCoupledHazardMath'],
    ['./dom-environmental-coupled-physics.js?v=20260914-3','domEnvironmentalCoupledPhysics'],
    ['./dom-global-operational-weather-field.js?v=20260914-8','domGlobalOperationalWeather'],
    ['./dom-wmo-gbon-station-supplement.js?v=20260914-2','domWmoGbonStations'],
    ['./dom-global-coverage-gap-audit.js?v=20260914-3','domGlobalCoverageGapAudit'],
    ['./dom-geographic-semantic-forensic-audit.js?v=20260914-3','domGeographicSemanticForensicAudit']
  ];
  science.forEach((x,i)=>loadScript(x[0],x[1],2200+i*420));
  const dashTimer=setTimeout(()=>{
    let css=document.querySelector('link[data-dom-dashboard-shell]');
    const loadDash=()=>loadScript('./dom-observatory-dashboard-shell.js?v=20260914-11','domObservatoryDashboardShell',150);
    if(css){if(css.sheet)loadDash();else css.addEventListener('load',loadDash,{once:true});return}
    css=document.createElement('link');css.rel='stylesheet';css.href='./dom-observatory-dashboard-shell.css?v=20260914-6';css.dataset.domDashboardShell='1';css.addEventListener('load',loadDash,{once:true});css.addEventListener('error',()=>console.error('D.O.M. dashboard stylesheet failed; dashboard shell not started'),{once:true});document.head.appendChild(css)
  },6200);timers.push(dashTimer)
}
