const DOMEnvironmentFusion=(()=>{
  ...
  function compoundIndex(x={}){
    ...
    const coupling=weighted([
      {value:interaction(domains.atmosphere.value,domains.ocean.value),weight:.22},
      {value:interaction(domains.atmosphere.value,domains.water.value),weight:.22},
      {value:interaction(domains.atmosphere.value,domains.fire.value),weight:.16},
      {value:interaction(domains.water.value,domains.ground.value),weight:.18},
      {value:interaction(domains.cryosphere.value,domains.water.value),weight:.12},
      {value:interaction(domains.fire.value,domains.ground.value),weight:.10}
    ]);
  ...
  function actionStage(x={}){
    ...
    if(x.officialWarning&&finite(x.localImpact)&&clamp(x.localImpact)>=.8)return{stage:'prepare-to-act',authority:'official-warning',text:'An official warning and high local-impact evidence are both present. Be ready to act immediately and follow local emergency instructions.'};
    if(finite(x.localImpact)&&clamp(x.localImpact)>=.78&&finite(x.evidence)&&clamp(x.evidence)>=.78)return{stage:'consider-early-departure',authority:'research',text:'Evidence is strong enough to consider early voluntary departure if local officials permit and travel remains safe. This is not an evacuation order.'};
  ...
  function explain(x={}){
    ...
    const a=actionStage({officialEvacuation:x.officialEvacuation,officialWarning:x.officialWarning,localImpact:impact,evidence});
