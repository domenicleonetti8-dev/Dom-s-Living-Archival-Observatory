...
for(let i=0;i<50000;i++){
  ...
  const gas=P.gasPpbToUgM3(ppb,mw,t,p);assert(gas&&gas.ugM3>=0);assert(P.gasPpbToUgM3(ppb*2,mw,t,p).ugM3>=gas.ugM3);assert(P.gasPpbToUgM3(ppb,mw,t,p*1.1).ugM3>=gas.ugM3);
  ...
  const area=between(0,1e10),thick=between(0,500),dens=between(500,1300),ice=P.iceMass(area,thick,dens);assert(ice&&rel(ice.massKg,area*thick*dens,1e-12));
  const ws=between(0,350),dir=between(-720,720),vec=P.windVector(ws,dir);assert(vec);assert(rel(Math.hypot(vec.uEastMs,vec.vNorthMs),ws/3.6,1e-12));
  ...
  const slick=P.slickMass(area,between(0,1),between(500,1300));assert(slick&&slick.massKg>=0&&rel(slick.massKg,slick.volumeM3*(slick.massKg/(slick.volumeM3||1)),1e-9));
  ...
  const Q=between(0.001,1e6),u=between(0.05,100),sy=between(0.1,10000),sz=between(0.1,10000),y=between(-20000,20000),z=between(0,10000),H=between(0,1000),pl=P.gaussianPlume(Q,u,sy,sz,y,z,H),plMirror=P.gaussianPlume(Q,u,sy,sz,-y,z,H);assert(pl&&pl.concentrationGm3>=0&&rel(pl.concentrationGm3,plMirror.concentrationGm3,1e-12));assert(P.gaussianPlume(Q*2,u,sy,sz,y,z,H).concentrationGm3>=pl.concentrationGm3);
  ...
  const sg=P.scalarGradient(lat,lon,pts,'temp');assert(sg);assert(Math.abs(sg.dValueDxPerKm-ax)<1e-7);assert(Math.abs(sg.dValueDyPerKm-ay)<1e-7);
