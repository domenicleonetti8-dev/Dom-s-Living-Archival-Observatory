...
assert.strictEqual(P.polygonAreaM2({type:'Polygon',coordinates:[[[0,0],[1,0],[999,1],[0,0]]]}),null,'invalid polygon coordinate must invalidate derived area');
assert.strictEqual(P.polygonAreaM2({type:'Polygon',coordinates:[[[0,0],[0,0],[0,0],[0,0]]]}),null,'degenerate polygon must not become zero physical area');
assert.strictEqual(P.flowWeightedMix([{concentration:10,flow:1},{flow:2}]),null,'missing concentration on a positive-flow stream must invalidate the mix');
assert.strictEqual(P.flowWeightedMix([{concentration:10,flow:1},{concentration:-1,flow:2}]),null,'negative concentration must not enter conservative mixing');
assert.strictEqual(P.flowWeightedMix([{concentration:10,flow:1},{flow:0}]).concentration,10,'zero-flow stream may be ignored because it contributes no mass or flow');
