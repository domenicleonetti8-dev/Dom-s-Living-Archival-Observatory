(()=>{
  ...
  function truthifySearchCopy(){
    const intent=document.querySelector('#intent');if(intent){intent.textContent=intent.textContent.replace(/live connectors/gi,'inline API connectors').replace(/federated research network/gi,'configured inline API connectors')}
  ...
  function truthifyObservatoryLinks(){const box=document.querySelector('#obs');if(!box)return;for(const a of box.querySelectorAll('a')){const small=a.querySelector('small');if(small)small.textContent='OFFICIAL PORTAL';const span=a.querySelector('span');if(span)span.textContent='Open official portal →'}}
