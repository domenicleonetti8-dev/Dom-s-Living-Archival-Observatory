(()=>{
  ...
  function render(){
    ...
    box.innerHTML=DEFINITIONS.map(x=>`<article><small>${esc(x.cat.toUpperCase())} · ${esc(x.mode)} · ${esc(x.contentClass)}</small><h3>${esc(x.name)}</h3><p>${esc(x.note)}</p><strong class="${stateClass(x.state)}">${esc(x.state)}${x.httpStatus?` · HTTP ${esc(x.httpStatus)}`:''}</strong>${x.lastAttempt?`<small>Last browser attempt: ${esc(new Date(x.lastAttempt).toLocaleTimeString())}${x.responseMs!=null?` · ${esc(x.responseMs)} ms`:''}</small>`:''}${x.error?`<small>${esc(x.error)}</small>`:''}</article>`).join('');
  }
