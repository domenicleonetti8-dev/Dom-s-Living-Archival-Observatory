...
function escapeHtml(v){return safeText(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c]))}
...
function installEarthImagery(){
  ...
  map.addLayer({id:'dom-earth-imagery-layer',type:'raster',source:'dom-earth-imagery',paint:{'raster-opacity':.92,'raster-fade-duration':0}},firstSymbol&&firstSymbol.id);
}
function toggleEarthImagery(){
  if(!map||!map.getLayer('dom-earth-imagery-layer'))return;
  earthImageryVisible=!earthImageryVisible;
  map.setLayoutProperty('dom-earth-imagery-layer','visibility',earthImageryVisible?'visible':'none');
  const b=$('#earthImagery');if(b)b.textContent=`Earth imagery: ${earthImageryVisible?'ON':'OFF'}`;
  $('#providerState').textContent=earthImageryVisible?'NASA Blue Marble Earth imagery active beneath source-backed D.O.M. observations. Baseline imagery is not live.':'Earth imagery hidden · geographic map and source-backed observations remain active.';
}
