...
function addAreaFeature(r){
  ...
  areaKeys.add(key);
  areaFeatures.push({type:'Feature',id:key,geometry:g,properties:{id,source:safeText(r.source||'Unknown'),type:r.type||'hazard',title:safeText(r.title||r.source||'Source geometry'),time:r.time||'',url:r.url||'',detail:safeText(r.detail||''),placement:'source-geometry',geometryRole:role,geometryType:g.type}});
  return true;
}
...
function installObservationLayers(){
  ...
  const polygonFilter=['in',['geometry-type'],['literal',['Polygon','MultiPolygon']]];
  const lineFilter=['in',['geometry-type'],['literal',['LineString','MultiLineString']]];
  if(!map.getLayer('dom-event-area-fill'))map.addLayer({id:'dom-event-area-fill',type:'fill',source:'dom-event-areas',filter:polygonFilter,paint:{'fill-color':['match',['get','type'],'alert',colorFor.alert,colorFor.hazard],'fill-opacity':0.14}});
  if(!map.getLayer('dom-event-area-line'))map.addLayer({id:'dom-event-area-line',type:'line',source:'dom-event-areas',filter:polygonFilter,paint:{'line-color':['match',['get','type'],'alert',colorFor.alert,colorFor.hazard],'line-width':2,'line-opacity':0.85}});
...
async function pollEONET(){const id='eonet';setSource(id,'NASA EONET events','LOADING',0,'open natural events');try{const j=await getJSON('https://eonet.gsfc.nasa.gov/api/v3/events?status=open&limit=500');let points=0,geometries=0;for(const e of j.events||[]){const g=(e.geometry||[]).at(-1);if(!g)continue;const common={id:`eonet:${e.id}`,source:'NASA EONET',type:'hazard',title:e.title,time:g.date||null,url:(e.sources||[])[0]?.url||null,detail:(e.categories||[]).map(c=>c.title).join(', ')};if(g.type==='Point'&&Array.isArray(g.coordinates)){if(addRecord({...common,lat:g.coordinates[1],lon:g.coordinates[0]}))points++}else if(addAreaFeature({...common,geometry:g,geometryRole:'event-geometry'}))geometries++}setSource(id,'NASA EONET events','LIVE',points+geometries,`${points} point events · ${geometries} authoritative source geometries`)}catch(e){setSource(id,'NASA EONET events','FAILED',0,String(e.message||e))}}
...
window.DOMEarthRuntime=Object.freeze({refreshFabric,records:()=>records.slice(),areaFeatures:()=>areaFeatures.slice(),sourceStates:()=>[...sourceStates.values()],validLatLon,googleUrl,sourceGeometryTypes:()=>[...SOURCE_GEOMETRY_TYPES]});
