...
function addAreaFeature(r){
  ...
  const id=safeText(r.id||`${r.source}:area:${areaFeatures.length}`);
  if(areaKeys.has(id))return false;
  areaKeys.add(id);
  areaFeatures.push({type:'Feature',id,geometry:g,properties:{id,source:safeText(r.source||'Unknown'),type:r.type||'hazard',title:safeText(r.title||r.source||'Area event'),time:r.time||'',url:r.url||'',detail:safeText(r.detail||''),placement:'source-geometry'}});
  return true;
}
...
function renderSourceFabric(){
  ...
  if(!box)return;
  box.innerHTML=[...sourceStates.values()].map(s=>`<article class="source-card"><div><strong>${escapeHtml(s.label)}</strong><span>${escapeHtml(s.detail)}</span></div><b class="source-${escapeHtml(String(s.state).toLowerCase())}">${escapeHtml(s.state)}${Number.isFinite(Number(s.count))?` · ${Number(s.count).toLocaleString()}`:''}</b></article>`).join('');
}
function toGeoJSON(){return{type:'FeatureCollection',features:records.map(r=>({type:'Feature',id:r.id,geometry:{type:'Point',coordinates:[r.lon,r.lat]},properties:{id:r.id,source:r.source,type:r.type,title:r.title,time:r.time||'',url:r.url||'',detail:r.detail}}))}}
function toAreaGeoJSON(){return{type:'FeatureCollection',features:areaFeatures.slice()}}
function updateMap(){if(!map)return;const src=map.getSource('dom-observations');if(src)src.setData(toGeoJSON());const areas=map.getSource('dom-event-areas');if(areas)areas.setData(toAreaGeoJSON());updateGoogleMarkers()}
function installObservationLayers(){
  ...
  if(!map.getLayer('dom-event-area-fill'))map.addLayer({id:'dom-event-area-fill',type:'fill',source:'dom-event-areas',paint:{'fill-color':['match',['get','type'],'alert',colorFor.alert,colorFor.hazard],'fill-opacity':0.14}});
  if(!map.getLayer('dom-event-area-line'))map.addLayer({id:'dom-event-area-line',type:'line',source:'dom-event-areas',paint:{'line-color':['match',['get','type'],'alert',colorFor.alert,colorFor.hazard],'line-width':2,'line-opacity':0.85}});
}
...
function toggleEarthImagery(){earthImageryVisible=!earthImageryVisible;if(map?.getLayer('dom-earth-imagery-layer'))map.setLayoutProperty('dom-earth-imagery-layer','visibility',earthImageryVisible?'visible':'none');const b=$('#earthImagery');if(b)b.textContent=`Earth imagery: ${earthImageryVisible?'ON':'OFF'}`;const s=$('#providerState');if(s)s.textContent=earthImageryVisible?'NASA Blue Marble geographic Earth active · source-backed D.O.M. observations remain anchored by latitude/longitude. Baseline imagery is not live.':'Earth imagery hidden · geographic labels and source-backed observations remain active.'}

async function pollUSGS(){const id='usgs';setSource(id,'USGS earthquakes','LOADING',0,'M2.5+ · past day');try{const j=await getJSON('https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/2.5_day.geojson');let n=0;for(const f of j.features||[]){const c=f?.geometry?.coordinates;if(!Array.isArray(c)||!validLatLon(c[1],c[0]))continue;if(addRecord({id:`usgs:${f.id}`,source:'USGS',type:'earthquake',title:f.properties?.title||'Earthquake',lat:c[1],lon:c[0],time:f.properties?.time||null,url:f.properties?.url||null,detail:`M${f.properties?.mag??'?'}`}))n++}setSource(id,'USGS earthquakes','LIVE',n,'M2.5+ · past day')}catch(e){setSource(id,'USGS earthquakes','FAILED',0,String(e.message||e))}}
async function pollEONET(){const id='eonet';setSource(id,'NASA EONET events','LOADING',0,'open natural events');try{const j=await getJSON('https://eonet.gsfc.nasa.gov/api/v3/events?status=open&limit=500');let points=0,areas=0;for(const e of j.events||[]){const g=(e.geometry||[]).at(-1);if(!g)continue;const common={id:`eonet:${e.id}`,source:'NASA EONET',type:'hazard',title:e.title,time:g.date||null,url:(e.sources||[])[0]?.url||null,detail:(e.categories||[]).map(c=>c.title).join(', ')};if(g.type==='Point'&&Array.isArray(g.coordinates)){if(addRecord({...common,lat:g.coordinates[1],lon:g.coordinates[0]}))points++}else if(addAreaFeature({...common,geometry:g}))areas++}setSource(id,'NASA EONET events','LIVE',points+areas,`${points} point events · ${areas} source geometries`)}catch(e){setSource(id,'NASA EONET events','FAILED',0,String(e.message||e))}}
async function pollNDBC(){const id='ndbc';setSource(id,'NOAA NDBC stations','LOADING',0,'active buoy/station inventory');try{const text=await getText('https://www.ndbc.noaa.gov/activestations.xml');const doc=new DOMParser().parseFromString(text,'application/xml');let n=0;for(const st of [...doc.querySelectorAll('station')]){const lat=Number(st.getAttribute('lat')),lon=Number(st.getAttribute('lon'));if(addRecord({id:`ndbc:${st.getAttribute('id')}`,source:'NOAA NDBC',type:'station',title:st.getAttribute('name')||st.getAttribute('id'),lat,lon,url:'https://www.ndbc.noaa.gov/',detail:'marine / meteorological station'}))n++}setSource(id,'NOAA NDBC stations','LIVE',n,'active inventory only · not live measurement values')}catch(e){setSource(id,'NOAA NDBC stations','FAILED',0,String(e.message||e))}}
async function pollCOOPS(){const id='coops';setSource(id,'NOAA CO-OPS stations','LOADING',0,'water-level stations');try{const j=await getJSON('https://api.tidesandcurrents.noaa.gov/mdapi/prod/webapi/stations.json?type=waterlevels');let n=0;for(const st of j.stations||[]){if(addRecord({id:`coops:${st.id}`,source:'NOAA CO-OPS',type:'station',title:st.name||st.id,lat:st.lat,lon:st.lng,url:st.self||'https://tidesandcurrents.noaa.gov/',detail:'water-level station inventory'}))n++}setSource(id,'NOAA CO-OPS stations','LIVE',n,'station inventory · live measurement values are separate')}catch(e){setSource(id,'NOAA CO-OPS stations','FAILED',0,String(e.message||e))}}
async function pollNWS(){const id='nws';setSource(id,'NOAA/NWS active alerts','LOADING',0,'official alert source geometry');try{const j=await getJSON('https://api.weather.gov/alerts/active');let points=0,areas=0,unlocated=0;for(const f of j.features||[]){const g=f?.geometry,p=f?.properties||{},common={id:`nws:${f.id||p.id||crypto.randomUUID()}`,source:'NOAA/NWS',type:'alert',title:p.headline||p.event||'Active alert',time:p.sent||p.effective||null,url:p['@id']||f.id||null,detail:p.event||'alert'};if(g?.type==='Point'&&Array.isArray(g.coordinates)){if(addRecord({...common,lat:g.coordinates[1],lon:g.coordinates[0]}))points++}else if(addAreaFeature({...common,geometry:g}))areas++;else unlocated++}setSource(id,'NOAA/NWS active alerts','LIVE',points+areas,`${points} point alerts · ${areas} authoritative polygons · ${unlocated} without geometry`)}catch(e){setSource(id,'NOAA/NWS active alerts','FAILED',0,String(e.message||e))}}
async function loadBroker(){const base=window.DOMSRuntimeConfig?.brokerUrl;if(!base){setSource('broker','D.O.M. observation broker','REGISTERED',0,'not configured in this deployment');return}setSource('broker','D.O.M. observation broker','LOADING',0,base);try{const root=base.replace(/\/$/,'');const [obs,sources]=await Promise.all([getJSON(`${root}/v1/observations`,15000),getJSON(`${root}/v1/sources`,15000)]);const rows=Array.isArray(obs)?obs:Array.isArray(obs.records)?obs.records:[];let n=0;for(const r of rows){const lat=r.latitude??r.lat,lon=r.longitude??r.lon;if(addRecord({id:`broker:${r.id||r.observationId||r.sourceId||n}`,source:r.sourceAgency||r.agency||r.network||r.sourceId||'D.O.M. broker',type:r.officialAlert?'alert':'station',title:r.title||r.name||r.stationName||r.sourceId||'Broker observation',lat,lon,time:r.observedAt||r.observed_at||null,url:r.sourceUrl||r.url||null,detail:r.variable?`${r.variable}${r.measurementValue!=null?`: ${r.measurementValue}${r.measurementUnit?` ${r.measurementUnit}`:''}`:''}`:(r.modality||r.platformClass||'broker record')}))n++}const ss=Array.isArray(sources.sources)?sources.sources:[];const active=ss.filter(x=>x.status==='active').length,stale=ss.filter(x=>x.status==='stale').length,failed=ss.filter(x=>x.status==='error').length;setSource('broker','D.O.M. observation broker',failed?'DEGRADED':stale?'STALE':'LIVE',n,`${active} active adapters · ${stale} stale · ${failed} failed · ${n} geolocated records`) }catch(e){setSource('broker','D.O.M. observation broker','FAILED',0,String(e.message||e))}}
async function refreshFabric(){clearRecords();await Promise.allSettled([pollUSGS(),pollEONET(),pollNDBC(),pollCOOPS(),pollNWS(),loadBroker()]);updateMap()}
...
function updateGoogleMarkers(){if(!googleMap||!window.google?.maps)return;googleMarkers.forEach(m=>m.setMap(null));googleMarkers=[];for(const r of records.slice(0,2500)){googleMarkers.push(new google.maps.Marker({position:{lat:r.lat,lng:r.lon},map:googleMap,title:r.title}))}}
...
map.on('load',()=>{installEarthImagery();installObservationLayers();refreshFabric();const s=$('#providerState');if(s)s.textContent='NASA Blue Marble geographic Earth active · source-backed D.O.M. observations remain anchored by latitude/longitude. Baseline imagery is not live.'});
map.on('click','dom-observations',e=>{const f=e.features?.[0];if(!f)return;const p=f.properties||{};new maplibregl.Popup().setLngLat(e.lngLat).setHTML(`<strong>${escapeHtml(p.title)}</strong><br><small>${escapeHtml(p.source)} · ${escapeHtml(p.detail)}</small>${p.time?`<br><small>${escapeHtml(p.time)}</small>`:''}${p.url?`<br><a target="_blank" rel="noopener noreferrer" href="${escapeHtml(p.url)}">official source ↗</a>`:''}`).addTo(map)});
map.on('click','dom-event-area-fill',e=>{const f=e.features?.[0];if(!f)return;const p=f.properties||{};new maplibregl.Popup().setLngLat(e.lngLat).setHTML(`<strong>${escapeHtml(p.title)}</strong><br><small>${escapeHtml(p.source)} · authoritative source geometry</small>${p.detail?`<br><small>${escapeHtml(p.detail)}</small>`:''}${p.time?`<br><small>${escapeHtml(p.time)}</small>`:''}${p.url?`<br><a target="_blank" rel="noopener noreferrer" href="${escapeHtml(p.url)}">official source ↗</a>`:''}`).addTo(map)});
...
window.DOMEarthRuntime=Object.freeze({refreshFabric,records:()=>records.slice(),areaFeatures:()=>areaFeatures.slice(),sourceStates:()=>[...sourceStates.values()],validLatLon,googleUrl});
