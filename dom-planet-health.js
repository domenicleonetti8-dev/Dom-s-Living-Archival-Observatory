const DOMPlanetHealth=(()=>{
  ...
  function state(input={}){const domains={climate:weightedScore(input.climate||[]),ocean:weightedScore(input.ocean||[]),hydrology:weightedScore(input.hydrology||[]),cryosphere:weightedScore(input.cryosphere||[]),forests:weightedScore(input.forests||[]),reefs:weightedScore(input.reefs||[]),wildfire:weightedScore(input.wildfire||[]),atmosphere:weightedScore(input.atmosphere||[]),biodiversity:weightedScore(input.biodiversity||[]),geophysical:weightedScore(input.geophysical||[])};const priors={climate:.17,ocean:.13,hydrology:.10,cryosphere:.09,forests:.11,reefs:.08,wildfire:.07,atmosphere:.10,biodiversity:.08,geophysical:.07};let n=0,d=0,globalCoverage=0,observedPriorWeight=0;const qualified=[];for(const[k,v]of Object.entries(domains)){const w=priors[k],cov=clamp(v.coverage);globalCoverage+=w*cov;if(v.value==null)continue;observedPriorWeight+=w;const ew=w*cov;if(ew>0){n+=v.value*ew;d+=ew}if(cov>=.5)qualified.push(k)}const stress=d?n/d:null,coverage=clamp(globalCoverage),provisionalHealth=stress==null?null:1-stress,displayQualified=provisionalHealth!=null&&coverage>=.6&&qualified.length>=6&&observedPriorWeight>=.6;return{stress,provisionalHealth,health:displayQualified?provisionalHealth:null,coverage,observedPriorWeight,effectiveGlobalWeight:d,displayQualified,qualifiedDomains:qualified,priors,method:'Domain stress means are evidence-weighted internally; the global composite is prior-weighted and then multiplied by each domain evidence coverage. Domains with weak coverage therefore cannot contribute at full prior weight.',domains}}
  ...
  function forecastWindow({series=[],threshold,direction='above',minPoints=8,minSpanYears=2,minR2=.55,maxHorizonYears=100}={}){
    ...
    return{kind:'statistical-threshold-window',estimateT:t,lowerT:t-1.96*sigmaT,upperT:t+1.96*sigmaT,slope,r2,reducedChi2,sigmaT,count:pts.length,spanYears:span,referenceT:xbar,warning:'Weighted linear-regression threshold window only. It assumes supplied sigma values are meaningful and does not model temporal autocorrelation or structural/scenario uncertainty; authoritative scenario ensembles take precedence.'};
  }
