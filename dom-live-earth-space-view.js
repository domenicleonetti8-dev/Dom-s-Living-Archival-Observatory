...
const state={map:null,timer:0,frameTime:null,previousTime:null,layers:[],lastError:null,phase:'idle',depth:'orbit'};
...
function depth(z){return z<2.2?'orbit':z<5.2?'upper-atmosphere':z<7.6?'cloud-top':z<9.4?'inside-cloud':'below-cloud'}
function opacity(z){return z<2.2?.96:z<5.2?.90:z<7.6?.78:z<9.4?.48:.10}
function addFrame(p,t,slot){const m=state.map,sid=`dom-worldview-${p.id}-${slot}`,lid=`${sid}-atmosphere`;if(!m||!m.loaded?.())return null;try{if(m.getLayer?.(lid))m.removeLayer(lid);if(m.getSource?.(sid))m.removeSource(sid);m.addSource(sid,{type:'raster',tiles:[tile(p.layer,t)],tileSize:256,attribution:'NASA Worldview / GIBS'});const before=m.getLayer?.('dom-live-points-layer')?'dom-live-points-layer':undefined;m.addLayer({id:lid,type:'raster',source:sid,paint:{'raster-opacity':slot==='previous'?0:opacity(m.getZoom?.()??0),'raster-fade-duration':TRANSITION,'raster-saturation':.08,'raster-contrast':.04}},before);state.layers.push({id:lid,slot,product:p.id});return lid}catch(e){state.lastError=String(e?.message||e);return null}}
...
function setLOD(){const m=state.map;if(!m)return;const z=m.getZoom?.()??0,next=depth(z),changed=next!==state.depth;state.depth=next;for(const id of['dom-live-points-layer','dom-live-touch-layer','dom-eira-quake-origin','dom-live-pulse'])try{if(m.getLayer?.(id))m.setLayoutProperty(id,'visibility',z<2.2?'none':'visible')}catch(_){};for(const x of state.layers)try{if(m.getLayer?.(x.id))m.setPaintProperty(x.id,'raster-opacity',x.slot==='previous'?0:opacity(z))}catch(_){};if(changed)window.dispatchEvent(new CustomEvent('dom:atmosphere-depth-change',{detail:{depth:next,zoom:z,truth:next==='inside-cloud'?'VISUAL_DEPTH_FROM_OBSERVED_IMAGERY':'OBSERVED_IMAGERY_PRESENTATION',note:'Depth transition is presentation geometry; cloud pixels remain observation-derived'}}))}
function refresh(){const m=state.map;if(!m||!m.loaded?.())return false;const current=frameDate(1),previous=frameDate(2);if(current===state.frameTime&&state.layers.length){setLOD();return true}state.phase='transition';clean();state.previousTime=previous;state.frameTime=current;for(const p of PRODUCTS){addFrame(p,previous,'previous');addFrame(p,current,'current')}setLOD();state.phase='observed';window.dispatchEvent(new CustomEvent('dom:worldview-atmosphere-frame',{detail:{source:'NASA Worldview / GIBS',previousFrameTime:previous,frameTime:current,truth:'OBSERVED_FRAMES_WITH_RENDER_INTERPOLATION',interpolation:'visual cross-frame transition only; never classified as observation',gapPolicy:'transparent-underlay; no synthetic cloud fill',depthModel:'orbit > upper-atmosphere > cloud-top > inside-cloud > below-cloud',products:PRODUCTS.map(p=>p.layer)}}));return true}
...
window.DOMLiveEarthSpaceView=Object.freeze({attach,refresh,state:()=>({frameTime:state.frameTime,previousTime:state.previousTime,layers:state.layers.map(x=>x.id),phase:state.phase,depth:state.depth,lastError:state.lastError,products:PRODUCTS})});
