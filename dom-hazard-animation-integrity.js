...
function addExplicitHazards(){if(!map||!map.loaded()||!map.getSource('dom-live-points'))return;const add=(id,def)=>{try{if(!map.getLayer(id))map.addLayer(def)}catch(_){}};
 const event=['==',['get','kind'],'event'];
...
function settle(){const now=performance.now();if(now-lastTune<120)return;lastTune=now;removeLegacy();tuneRaster();syncTerrain();addExplicitHazards()}
function attach(m){map=m;terrainOn=!!map.getTerrain?.();removeLegacy();tuneRaster();syncTerrain();addExplicitHazards();try{map.on('zoomend',settle);map.on('moveend',settle);map.on('styledata',()=>setTimeout(settle,0));map.on('idle',()=>{removeLegacy();tuneRaster();addExplicitHazards()})}catch(_){}}
window.addEventListener('dom:map-ready',e=>{if(e.detail?.map)attach(e.detail.map)});
for(const n of ['dom:hazard-extension','dom:hazard-refresh','dom:verified-global-events'])window.addEventListener(n,()=>setTimeout(()=>{removeLegacy();tuneRaster();addExplicitHazards()},0));
window.addEventListener('pageshow',()=>setTimeout(settle,0));
window.DOMHazardAnimationIntegrity=Object.freeze({state:()=>({attached:!!map,continuousLegacyAnimation:false,terrainOn,orbitalTerrainSuppressed:!!map&&map.getZoom()<4.35,explicitGlobalHazards:true}),ensure:()=>{settle();return !!map}});
})();