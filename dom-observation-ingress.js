const DOMObservationIngress=(()=>{
  ...
  function hasMeasurement(raw={}){if(raw.measurement!==null&&raw.measurement!==undefined&&raw.measurement!=='')return true;if(Array.isArray(raw.measurements)&&raw.measurements.length>0)return true;if(finite(raw.mag)||finite(raw.magnitude))return true;if(raw.storm&&typeof raw.storm==='object'&&(finite(raw.storm.windKts)||finite(raw.storm.windMph)||finite(raw.storm.pressureHpa)))return true;return false}
  function observationStatus(raw={}){const explicit=String(raw.observationStatus||'').toLowerCase();if(['modeled','forecast','projection','aggregated'].includes(explicit))return explicit;if(explicit==='observed')return hasMeasurement(raw)?'observed':'reported';if(['reported','unknown'].includes(explicit))return explicit;if(raw.officialAlert===true)return String(raw.certaintyText||'').toLowerCase()==='observed'&&hasMeasurement(raw)?'observed':'reported';const modality=String(raw.modality||'').toLowerCase();if(/event-aggregation|reanalysis|model|projection|forecast/.test(modality))return /reanalysis|model/.test(modality)?'modeled':/projection/.test(modality)?'projection':/forecast/.test(modality)?'forecast':'aggregated';if(/seismic|sensor|station|gauge|buoy|float|thermal|radar|satellite/.test(modality))return hasMeasurement(raw)?'observed':'reported';return raw.observed===true&&hasMeasurement(raw)?'observed':'unknown'}
  function normalize(raw={},context={}){
    ...
    const observedAt=iso(raw.observedAt||raw.time),receivedAt=iso(raw.receivedAt)||new Date().toISOString(),expiresAt=iso(raw.expiresAt||raw.expires);
  ...
  function dedupe(records=[]){const m=new Map();for(const raw of records){const n=raw&&raw.schema==='dom.observation.v1'?{ok:true,record:{...raw,observationStatus:observationStatus(raw)},errors:[]}:normalize(raw);if(!n.ok)continue;const r=n.record,key=`${r.lineageId}|${r.sourceId}|${r.observedAt||''}`;const old=m.get(key);if(!old||String(r.receivedAt||'')>String(old.receivedAt||''))m.set(key,r)}return[...m.values()]}
  ...
  function summary(records=[]){const rows=dedupe(records),lineages=new Set(rows.map(r=>r.lineageId).filter(Boolean)),agencies=new Set(rows.map(r=>r.sourceAgency).filter(Boolean));return{records:rows.length,lineages:lineages.size,agencies:agencies.size,geolocated:rows.filter(r=>r.lat!==null&&r.lon!==null).length,officialAlerts:rows.filter(r=>r.officialAlert).length,observed:rows.filter(r=>r.observationStatus==='observed').length,reportedWithoutMeasurement:rows.filter(r=>r.observationStatus==='reported'&&!hasMeasurement(r)).length}}
  return{normalize,dedupe,toSensor,summary,validLatLon,hasMeasurement,observationStatus};
})();
