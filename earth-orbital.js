...
const wms=d=>`${GIBS}?SERVICE=WMS&REQUEST=GetMap&VERSION=1.1.1&LAYERS=${LAYER}&STYLES=&FORMAT=image/jpeg&TRANSPARENT=false&SRS=EPSG:4326&BBOX=-180,-90,180,90&WIDTH=2048&HEIGHT=1024&TIME=${d}`;
...
const globe=new THREE.Mesh(new THREE.SphereGeometry(1,256,160),new THREE.MeshBasicMaterial({color:0xffffff}));scene.add(globe);
...
function loadObservation(i=0){
 if(i>=dates.length){setState('Geographical Earth · observed satellite texture unavailable · geographic terrain remains available.');return;}
 const d=dates[i];
 loader.load(wms(d),t=>{t.colorSpace=THREE.SRGBColorSpace;t.wrapS=THREE.RepeatWrapping;t.wrapT=THREE.ClampToEdgeWrapping;t.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());globe.material.map=t;globe.material.color.set(0xffffff);globe.material.needsUpdate=true;setState(`Geographical Earth · NASA VIIRS ${d} continuous spherical observation surface · seamless poles/dateline · pinch inward to descend into geographic terrain.`);},undefined,()=>loadObservation(i+1));
}
loadObservation();
...
function move(e){if(!dragging||paused)return;const dx=e.clientX-lastX,dy=e.clientY-lastY;lastX=e.clientX;lastY=e.clientY;globe.rotation.y+=dx*.006;globe.rotation.x=Math.max(-1.45,Math.min(1.45,globe.rotation.x+dy*.006));atmosphere.rotation.copy(globe.rotation)}
...
window.DOM_ORBITAL={reset(){paused=false;host.classList.remove('detail-active');document.getElementById('earthMap')?.classList.remove('active');globe.rotation.set(0,0,0);atmosphere.rotation.set(0,0,0);zoom=3.15;camera.position.z=zoom;setState('Geographical Earth · continuous spherical observed Earth active.');},refresh(){loadObservation(0);},resume(){paused=false;zoom=1.62;camera.position.z=zoom;}};
