const DOMGlobalSensorGlobe=(()=>{
  ...
  function normalizeSensor(raw={}){
    const lat=Number(raw.lat),lon=Number(raw.lon),activation=clamp(raw.activation);
    return{
  ...
  function cluster(sensors,level=2){
    const m=new Map();for(const s of sensors||[]){if(s.lat==null||s.lon==null)continue;const k=clusterKey(s,level),r=m.get(k)||{key:k,count:0,lat:0,lon:0,maxActivation:0,modalities:new Set()};r.count++;r.lat+=s.lat;r.lon+=s.lon;r.maxActivation=Math.max(r.maxActivation,s.activation||0);if(s.modality)r.modalities.add(s.modality);m.set(k,r)}
    return [...m.values()].map(r=>({key:r.key,count:r.count,lat:r.lat/r.count,lon:r.lon/r.count,maxActivation:r.maxActivation,band:activationBand(r.maxActivation),modalities:[...r.modalities]}));
  }
  function renderPlan({altitudeKm,sensors=[],events=[],viewport}={}){
    const lod=lodForAltitudeKm(altitudeKm),visible=viewportFilter(sensors,{...(viewport||{}),max:lod.maxSensors});
    return{lod,sensors:lod.level<=2?cluster(visible,lod.level):visible,events:(events||[]).map(normalizeEvent)};
  }
  return{ACTIVATION,activationBand,normalizeSensor,normalizeEvent,lodForAltitudeKm,viewportFilter,cluster,renderPlan};
})();
