const DOMHazardObservationBridge=(()=>{
  function eventToObservation(e={}){
    const raw={id:e.id,sourceId:e.id,sourceAgency:e.source,agency:e.source,network:e.sourceType==='official-alert'?'official-alerts':e.source,lineageId:e.lineageId,kind:e.kind,modality:e.modality,lat:e.lat,lon:e.lon,locationPrecision:e.locationPrecision,time:e.time,observedAt:e.time,receivedAt:new Date().toISOString(),url:e.url,sourceUrl:e.url,authoritative:e.authoritative===true,officialAlert:e.officialAlert===true,title:e.title,severityText:e.severityText,certaintyText:e.certaintyText,urgencyText:e.urgencyText,mag:e.mag,geometry:e.geometry,quality:Number.isFinite(Number(e.quality))?Number(e.quality):null,freshness:Number.isFinite(Number(e.fresh))?Number(e.fresh):null,corroboration:Number.isFinite(Number(e.corr))?Number(e.corr):null,persistence:Number.isFinite(Number(e.trend))?Number(e.trend):null};
