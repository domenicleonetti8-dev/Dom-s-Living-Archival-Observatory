const DOMSHazardVoice=(()=>{
  ...
  const preferred=[
    {name:'Daniel',lang:/^en[-_]GB$/i},
    {name:'Arthur',lang:/^en[-_]GB$/i},
    {name:'Daniel',lang:/^en/i},
    {name:'Arthur',lang:/^en/i},
    {name:'Alex',lang:/^en/i},
    {name:'Aaron',lang:/^en/i},
    {name:'Fred',lang:/^en/i},
    {name:'Ralph',lang:/^en/i}
  ];
  function chooseVoice(){
    ...
    for(const p of preferred){
      S.voice=vs.find(v=>p.lang.test(String(v&&v.lang||''))&&new RegExp(`^${p.name}$`,'i').test(String(v&&v.name||'')));
      if(S.voice)break;
    }
    return S.voice;
  }
  ...
  async function speak(text,{force=false,tag='',autoEnable=false}={}){
    ...
    rememberTag(tag);
    const u=new SpeechSynthesisUtterance(speechText(text));u.rate=S.rate;u.pitch=S.pitch;u.volume=S.volume;u.lang=voice.lang||'en-GB';u.voice=voice;
  ...
  function state(){return{enabled:S.enabled,muted:S.muted,supported:supported(),speaking:!!S.active,queuedTags:S.tagOrder.length,voice:S.voice?S.voice.name:null,voiceLang:S.voice?S.voice.lang:null,britishMalePreferred:true}}
