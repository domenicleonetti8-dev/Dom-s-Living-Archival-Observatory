const DOMSHazardVoice=(()=>{
  const S={enabled:false,muted:false,lastSpoken:new Set(),voice:null,rate:.96,pitch:1,volume:1,active:null};
  ...
  function stopOwn(){if(!supported()||!S.active)return;try{speechSynthesis.cancel()}catch(_){ }S.active=null}
  ...
  function mute(v=true){S.muted=!!v;if(S.muted)stopOwn();return S.muted}
  function stop(){stopOwn()}
  function state(){return{enabled:S.enabled,muted:S.muted,supported:supported(),speaking:!!S.active}}
  if(supported()){if(typeof speechSynthesis.addEventListener==='function')speechSynthesis.addEventListener('voiceschanged',chooseVoice);else speechSynthesis.onvoiceschanged=chooseVoice;chooseVoice()}
